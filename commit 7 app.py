...
from shared import _load_events
import collections
import datetime
try:
    import pandas as pd
...
def show_main_app():
    ...
    with st.sidebar:
        ...
        for label, page_key in [("Home", "home"), ("Logistics", "logistics"), ("Analytics", "analytics"), ("About", "about")]:
    ...
    if page == "home":
        ...
        **Assistive Buttons** is a simple, accessible communication tool designed for the disabled and elderly.
        ...
        st.markdown('<h2>Logistics</h2>', unsafe_allow_html=True)
        events = _load_events() or HISTORY
        if not events:
            ...
            for e in events:
                ...
                try:
                    ...
                    dt = None
                rows.append({
                    'timestamp': ts, 'dt': dt, 'date': dt.date() if dt else None,
                    'button': e.get('button'), 'device_id': e.get('device_id') or 'unknown',
                    'text': e.get('text')
                })
            
            df = pd.DataFrame(rows) if pd else None
            min_date = min([r['date'] for r in rows if r['date']]) if any(r['date'] for r in rows) else None
            max_date = max([r['date'] for r in rows if r['date']]) if any(r['date'] for r in rows) else None
            
            if min_date and max_date:
                dr = st.date_input('Date range', value=(min_date, max_date), key='log_dr')
            else:
                dr = None
            
            if df is not None:
                mask = df['date'].between(dr[0], dr[1]) if dr else [True]*len(df)
                df_f = df[mask]
            else:
                df_f = [r for r in rows if not dr or (r['date'] and dr[0] <= r['date'] <= dr[1])]
            
            c1, c2 = st.columns(2)
            c1.metric('Total Events', len(df_f) if df is not None else len(df_f))
            c2.metric('Events Today', len(df_f[df_f['date'] == datetime.date.today()]) if df is not None else len([r for r in df_f if r['date'] == datetime.date.today()]))
            
            st.subheader('Recent Events')
            if df is not None:
                st.dataframe(df_f.sort_values('dt', ascending=False).head(50)[['timestamp', 'button', 'text']], use_container_width=True)
            
            st.subheader('Events by Button Type')
            if df is not None and not df_f.empty and alt:
                btn_data = df_f['button'].value_counts().reset_index()
                btn_data.columns = ['button', 'count']
                chart = alt.Chart(btn_data).mark_bar().encode(x='count:Q', y=alt.Y('button:N', sort='-x'), color='button:N')
        ...
        events = _load_events() or HISTORY
        if not events:
            st.info("No events to analyze yet.")
        else:
            rows = []
            for e in events:
                ts = e.get('timestamp')
                try:
                    ...
                    dt = None
                rows.append({'timestamp': ts, 'dt': dt, 'date': dt.date() if dt else None, 'hour': dt.hour if dt else None,
                             'button': e.get('button'), 'device_id': e.get('device_id') or 'unknown', 'language': e.get('language')})
            
            df = pd.DataFrame(rows) if pd else None
            
            buttons = sorted(list({r['button'] for r in rows if r['button']}))
            sel_buttons = st.multiselect('Filter buttons', buttons, default=buttons)
            
            if df is not None:
                df_f = df[df['button'].isin(sel_buttons)]
            else:
                df_f = [r for r in rows if r['button'] in sel_buttons]
            
            st.metric('Total Events (Filtered)', len(df_f) if df is not None else len(df_f))
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.subheader('Button Distribution')
                if alt and df is not None and not df_f.empty:
                    btn_data = df_f['button'].value_counts().reset_index()
                    btn_data.columns = ['button', 'count']
                    chart = alt.Chart(btn_data).mark_arc().encode(theta='count:Q', color='button:N')
                    st.altair_chart(chart, use_container_width=True)
                else:
                    if df is not None:
                        st.dataframe(df_f['button'].value_counts().reset_index(name='count'))
            
            with col2:
                st.subheader('Language Usage')
                if df is not None and not df_f.empty:
                    lang_data = df_f['language'].value_counts().reset_index()
                    lang_data.columns = ['language', 'count']
                    if alt:
                        chart = alt.Chart(lang_data).mark_bar().encode(x='count:Q', y=alt.Y('language:N', sort='-x'), color='language:N')
                        st.altair_chart(chart, use_container_width=True)
                    else:
            ...
            with col1:
                st.subheader('Events Over Time (Daily)')
                if alt and df is not None and not df_f.empty:
                    daily = df_f.groupby('date').size().reset_index(name='count').sort_values('date')
                    chart = alt.Chart(daily).mark_line(point=True).encode(x='date:T', y='count:Q', tooltip=['date:T', 'count:Q'])
                    st.altair_chart(chart, use_container_width=True)
            
            with col2:
                st.subheader('Events by Hour')
                if alt and df is not None and not df_f.empty:
                    hourly = df_f.groupby('hour').size().reset_index(name='count').sort_values('hour')
                    chart = alt.Chart(hourly).mark_bar().encode(x=alt.X('hour:Q', scale=alt.Scale(domain=[0, 23])), y='count:Q', color=alt.value('#667eea'))
                    st.altair_chart(chart, use_container_width=True)
